...
function BajarY_x_CantCarateres($inicio_y,$cantCaracteres,$altoLinea_bajar,$text)
{
	    $cantVecesBajarY = strlen($text)/$cantCaracteres;
		$cantVecesBajarY = round($cantVecesBajarY,0);
		...
		{
			$arrLineas[$o] = '';
			while(strlen($arrLineas[$o]) < $cantCaracteres)
			{
				$arrLineas[$o] .= " ".$arrText[$r];
				$r++;
				if( $r ==  sizeof($arrText) )
				{
					$flag = false;
		...
		for($i = 1 ; $i < $cantVecesBajarY ; $i++)
		{
			$inicio_y -= $altoLinea_bajar;
		}
		return $inicio_y;
}
function addTextLimiteAncho_x_CantCarateres($x,$inicio_y,$cantCaracteres,$altoLinea_bajar,$size,$text,$angle=0,$wordSpaceAdjust=0)
{
	    $cantVecesBajarYsinRound = strlen($text)/$cantCaracteres;
		$cantVecesBajarY = round($cantVecesBajarYsinRound,0); 
		
		if( $cantVecesBajarYsinRound > $cantVecesBajarY)
		{ 
		  $cantVecesBajarY += 1; 
		}
		...
		$this->addText($x,$inicio_y,$size,$arrLineas[0],0,0);
		...
		for($i = 1 ; $i < $cantVecesBajarY ; $i++)
		{
			...
			{
			$inicio_y -= $altoLinea_bajar;			
			$this->addText($x,$inicio_y,$size,$arrLineas[$i],$angle,$wordSpaceAdjust);
			}
		}
		return $inicio_y;
}
...
function addJpegFromFile($img,$x,$y,$w=0,$h=0){
  ...
  $tmp = get_magic_quotes_runtime();
  //@set_magic_quotes_runtime(0);
  ini_set('set_magic_quotes_runtime', false);
